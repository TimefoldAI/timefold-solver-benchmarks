package ai.timefold.solver.benchmarks.examples.machinereassignment.optional.score;
...
import ai.timefold.solver.benchmarks.examples.machinereassignment.domain.MrLocation;
import ai.timefold.solver.benchmarks.examples.machinereassignment.domain.MrMachine;
import ai.timefold.solver.benchmarks.examples.machinereassignment.domain.MrMachineCapacity;
import ai.timefold.solver.benchmarks.examples.machinereassignment.domain.MrNeighborhood;
...
public class MachineReassignmentIncrementalScoreCalculator
    ...
    public void resetWorkingSolution(
        ...
        for (MrProcessAssignment processAssignment : machineReassignment
                .getProcessAssignmentList()) {
            MrMachine originalMachine =
                    processAssignment.getOriginalMachine();
    ...
    private void
            insert(MrProcessAssignment processAssignment) {
    ...
    private void
            retract(MrProcessAssignment processAssignment) {
        MrMachine machine = processAssignment.getMachine();
        if (machine != null) {
            MrServiceScorePart serviceScorePart = serviceScorePartMap.get(processAssignment.getService());
    ...
    private class MrServiceScorePart {
        ...
        private MrServiceScorePart(MrService service) {
            ...
            List<MrNeighborhood> neighborhoodList =
                    machineReassignment.getNeighborhoodList();
        ...
        private void addProcessAssignment(
                MrProcessAssignment processAssignment) {
            // Spread constraints
            MrLocation location =
                    processAssignment.getLocation();
            ...
            if (neighborhoodProcessCount == 1) {
                for (MrService fromDependencyService : service
    ...
    private class MrMachineScorePart {
        ...
        public void initOriginalProcessAssignment(
                MrProcessAssignment processAssignment) {
        ...
        private void doBalancePenaltyCosts() {
            for (MrBalancePenalty balancePenalty : machineReassignment
    ...
    private class MrMachineCapacityScorePart {
        ...
        private void initOriginalProcessAssignment(
                MrProcessAssignment processAssignment) {
    ...
    public Collection<ConstraintMatchTotal<HardSoftScore>> getConstraintMatchTotals() {
        ...
        for (MrMachineScorePart machineScorePart : machineScorePartMap.values()) {
            ...
            for (Map.Entry<MrService, Integer> entry : machineScorePart.serviceBag
                    .entrySet()) {
